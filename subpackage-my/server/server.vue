	...
	<view class="page">
		...
		<block v-for="(item, index) in messagelist" :key="index">
			<template v-if="!item.isMe">
				<view class=" d-flex p-1 my-1">
					<u-avatar :size="48" :src="item.avatar"></u-avatar>
					<view class="ml-1">
						{{ item.nickName }}
			...
			<template v-else>
				<view class="d-flex j-end">
					<view class=" d-flex p-1 my-1 a-center">
						<view class="mr-1">
							<template v-if="item.type === 'message'">
								<view class="text">{{ item.text }}</view>
							</template>
							...
							<template v-else-if="item.type === 'audio'">
								<template v-if="item.tmpId === loadingIndex ? isloading[loadingIndex] : false">
...
export default {
			...
			uni.chooseVideo({
				...
				success: res =>{
					...
					_this.isloading.push(true)
					  var message = this.im.createAudioMessage({
						...
						var message = this.im.createImageMessage({
						 ...
						 this.im.sendMessage({
						      ...
						      onSuccess: function (message) {
								  ...
								  //发送成功
						          console.log('Message sent successfully.',message.payload.url);
						      },
						      onFailed: function (error) { //发送失败
						 								    console.log(error);
						      }
	...
	onLoad() {
		console.log(uni.getStorageSync('userInfo').id === 18);
